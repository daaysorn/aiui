  ...
  return (
    <article className="min-w-0 pb-10 md:pb-24">
      ...
      <header>
        ...
        <h1 className="mt-4 max-w-[18ch] font-heading text-3xl leading-tight font-semibold tracking-tight xs:text-4xl md:text-5xl">
          {title}
        </h1>
        ...
        <ol className="mt-3 flex flex-col gap-2">
          {sections.map((section, index) => (
            <li key={section.id}>
              <a
                href={`#${section.id}`}
                className="inline-flex min-w-0 items-baseline gap-2 text-sm text-muted-foreground no-underline transition-colors hover:text-foreground"
              >
      ...
      <div
        className={cn(
          ...
          "[&_a]:font-medium [&_a]:text-foreground [&_a]:no-underline [&_a:hover]:opacity-80",
      ...
      >
        {sections.map((section, index) => (
          ...
          >
            <h2 className="font-heading text-xl font-semibold tracking-tight text-foreground xs:text-2xl">
              {section.title}
            </h2>
